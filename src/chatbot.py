...
class FAQChatbot:
    ...
    def get_response(self, user_question, top_k=3, threshold=0.25):
        """
        Get chatbot response to user question
        
        Args:
            user_question (str): Raw user question
            top_k (int): Number of results to consider (IMPROVED: 3 for better matching)
            threshold (float): Minimum similarity threshold (IMPROVED: 0.25 for more matches)
...
if __name__ == "__main__":
    ...
    for question in test_questions:
        ...
        response = chatbot.get_response(question, top_k=3, threshold=0.25)
